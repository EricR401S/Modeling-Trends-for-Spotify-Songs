...
The data set used in this research is a subset of a larger spotify dataset[link] that contained xxx number of tracks. It contains x number of observations/ tracks and x number of variables for songs between 2010 and 2019 thus the 2010s decade. 
Our focus in this research aims to infer which features of music can measure the popularity of songs using songs fro the 2010s decade and predict where a song is explicit or not based on these features. 
...
The original dataset contains `r nrow(df)` rows, which are song tracks, and `r ncol(df)` variable columns. When cleaning the dataset, potential challenges faced were:
...
Fig 3: The tempo and loudness recipe for a popular song lies between 50-200 bpm for tempo units and -20 to 0 for loudness decibles.
