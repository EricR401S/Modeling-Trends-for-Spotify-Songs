...
df1 <- rename(df1,acousticnessorig = acousticness)
df1 <- rename(df1,danceabilityorig = danceability)
df1 <- rename(df1,energyorig= energy)
df1 <- rename(df1,instrumentalnessorig= instrumentalness)
df1 <- rename(df1,tempoorig= tempo)
df1 <- rename(df1,loudnessorig= loudness)
df1 <- rename(df1,speechinessorig= speechiness)

#df1$acousticnessorig <- df1$acousticness
#df1$danceabilityorig <- df1$danceability
#df1$energyorig <- df1$energy
#df1$instrumentalnessorig <- df1$instrumentalness
#df1$tempoorig <-df1$tempo
#df1$loudnessorig <- df1$loudness
#df1$speechinessorig <- df1$speechiness


#divide first
df1$danceability0.1 <- (df1$danceabilityorig)*10
df1$speechiness0.1 <- (df1$speechinessorig)*10
df1$energy0.001 <- (df1$energyorig)*1000
df1$instrumentalnes0.001 <- (df1$instrumentalnessorig)*1000


#centered
df1$acousticness <- scale(df1$acousticnessorig, scale = FALSE)
...
df1$tempo <- scale(df1$tempoorig, scale = FALSE)
df1$loudness <- scale(df1$loudnessorig, scale = FALSE)
...
summarydf1=summ(scaled1, digits = 2, pvals = FALSE)
...
From the above model results, acousticness, danceability, energy, instrumentalness, loudness, speechiness and tempo are all significant predictors of a song's popularity score at the  $\alpha = 0.05$ significance level ($p< 0.001$). For mean value  of acousticness, danceability, energy, instrumentalness, tempo, loudness and speechiness, the popularity score of a track is 39.
For every 1 unit increase in acousticness and loudness, a song's average popularity will increase by 1.79 and 0.98 respectively. For every 0.1 unit increase in danceability and speechiness, the song's average popularity will increase by 1.17 and 0.64 respectively, and, for every 0.001 unit increase in a song's energy and instrumentalness, a song's average popularity will decrease by 0.02 in all cases holding all other factors constant. Additionally the model fit has $R^2$ value of 0.1727, meaning that almost 17% of the change in a song's popularity is explained by these predictors. We are 95% confident that the intervals (*see Appendix 3*) contain the true value for the significant predictors. The predictors in this model were found to be important in the order visualized below with significance cut off at acousticness.
...
tvalues <- as.data.frame(abs(summary(scaled1)$coefficients[,3]))
name <- c("acousticness","danceability","energy","instrumentalness","tempo","loudness","speechiness")
tdf <- data.frame(name,tvalues[2:8,])
...
knitr::kable((round(results,2)), caption =  "Odds Ratio and Confidence intervals") %>%
...
* **Normality of residuals**: The points on the quantile-quantile plot are mostly clustered along the 45 degree line, although a minimal amount of points trail off at the beginning and end of the line. 
<ul>
...
While analyzing popularity,we found that instrumentalness is the most important attribute influencing the popularity of the song in 2010s decade. A track with high levels of instrumentalness is likely to be least appealing to the audience followed by energy, loudness, danceability, speechiness and acousticness in that or- der. For a track’s popularity to be high, instrumentalness and energy, the most significant musical attributes should have low values. A track with high levels of acousticness, danceability, loudness and speechiness is likely to have a higher popularity score. Moreover, loudness was the predictor with the highest coefficient, which is not surprising. During music mastering, adjusting loudness commands high prioritization due to the human ear’s sensitivity(Sage Audio).

Based on the second research result, we conclude that the four chosen predictors, danceability, energy, speechiness and tempo, are indeed related to and can predict explicitness to some extent. Overall, the model at the 0.5 threshold is a good fit with an good, overall accuracy score, but a bad success rate at identifying explicit songs. However, when adjusting the cutoff to a best threshold of 0.087, the overall accuracy dropped, but the ability to predict explicit songs rose significantly, at the trade-off cost of less accuracy when classifying non-explicit songs. We conclude that we need better, more robust predictors to have better accuracy. Yet, we still remain with a pressing question. Why do we prioritize correctly classifying explicit songs? An ill classified explicit song should not play in a setting for children. Conversely, a misidentified, non-explicit song should not be penalized and lose potential profits by not being allowed to play in a general, public setting. Our model should be as accurate as possible to account for these scenarios.
...
For the first research question, the predictors chosen were only able to explain an 31% variable in popularity. This indicates a possibility of other significant predictors, which have not been taken into account for the analysis. For example, besides the attributes of a track, popularity could also be quite dependent largely on who is the artist. Secondly, the different genres have different combinations of musical attributes that make them popular within their respective frameworks. Therefore, this analysis doesn’t integrate genre classification into its context. Lastly, we initially theorized that energy and tempo were both statistically significant, but only tempo was not, according to the results. Also, even though it seems that popular songs should have higher energy levels to drive higher engagement with the audience, our model analysis suggests otherwise. These discrepancies might indicate a possibility of an interaction term, which we haven’t considered in the analysis.
For the second research question, our model is weak at distinguishing explicit from non-explicit songs because of the unequal distribution of the two categories in out dataset, which is reflective of real word patterns in regards to music consumption. The majority of songs are simply non-explicit; however, such distribution might hinder the training process when desiring to correctly categorize explicit and non-explicit songs. Secondly, more or better predictors may have been needed for this particular research question, such as lyrics, genre, popularity, duration, amongst others. Thirdly, Spotify likely employs the use of speech recognition models to flag the explicitness of songs, but we used statistical methods. Thus, the research where we used the four predictors to statistically predict the nature of a such a “latent” variable may have already been fraught with much difficulty since the beginning. To conclude, these results do prove that any variant of our model, even after adjusting the thresholds, is insufficient for predicting the explicitness of songs (there are 19% of errors in predicting for the former and 28% for the latter), but, at least, it is successful overall when classifying the explicitness and non-explicitness of songs.
For both questions, there are two glaring limitations that were overlooked during the process of this study. First and foremost, Spotify translated a numerous set of musical attributes into a few variables for stream- ing algorithms. This means an immeasurable amount of musical information was lost, such as silences, crescendos, instrument classes, among others. Even singer vocal types could have served as a predictor. Key changes in a song could have been another predictor. For the second question specifically, an oversight was attempting to predict explicitness while overlooking the fundamental human element. Songwriters are the people who are making songs explicit and non-explicit. Maybe predictors can be added that address the types of songwriters and the destinations for these tracks, which will be addressed in recommendations.

=======
While analyzing popularity,we found that *instrumentalness* is the most important attribute influencing the popularity of the song in 2010s decade. A track with high levels of instrumentalness is likely to be least appealing to the audience followed by *energy*, *loudness*, *danceability*, *speechiness* and *acousticness* in that order. For a track's popularity to be high, instrumentalness and energy, the most significant musical attributes should have low values. A track with high levels of acousticness, danceability, loudness and speechiness is likely to have a higher popularity score. Moreover, loudness was the predictor with the highest coefficient, which is not surprising. During music mastering, adjusting loudness commands high prioritization due to the human ear's sensitivity(Sage Audio). 
...
For our second research question, we can also incorporate predictors like language, duration, and genre and even interaction terms to aid the predictive power of the model. For example, an interaction term could be speechiness and tempo, given that trap songs follow certain tempo and speechiness levels (Burchell, 2019). A final recommendation, specifically due to the hurdles faced with this research question, is accessing another Spotify dataset that may contain more insightful predictors for this particular response variable. In this same thread of thinking, a way to address the songwriters who make these songs explicit and non-explicit is by adding variables that code their record labels, since the number of labels is finite when compared to the amount of songwriters. Variable ideas like these can help make the predictions more accurate when we choose to refit the model.
=======
For the first research question, in future studies, we can achieve better explanatory power by including more predictors crucial to understanding the popularity of the tracks, which may have not been included in this investigation despite our most careful approach when selecting variables with prior domain knowledge. We propose the inclusion of duration, genres as it may give a better understanding of how the popularity can be inferred. The time period of the research can be expanded to incorporate 1990s and 2000s decade for a better understanding of how the significance of musical attributes has changed over a period of time. We may also check the model for possible interaction terms that may influence the outcome variable, popularity, and subsequently improve the explanatory power of the model. 
...
>>>>>>> Stashed changes
As for other avenues of study, there are some variables that could have been relatively interesting in terms of prediction or inference. To illustrate, one could predict the scale of or duration or tempo of a song based on the popularity, in addition to other predictors. There are many possibilities, but some predictors are not good choices. Time signature was improperly coded and has an extremely limited set of plausible values (lacks signatures like 6/8). Another suggestion is investigating the datasets of other streaming giants, like Apple music, to see how they translate musical features into streaming metrics.
...
confinterval <- confint(scaled1, level = 0.95)
