...
For this study, we subset a [Spotify dataset](https://www.kaggle.com/datasets/yamaerenay/spotify-dataset-19212020-600k-tracks),with an assortment of descriptors focusing on the 2010s decade to investigate which qualities have been key to making songs popular and if they can be used to classify explicit and non explicit music, a phenomenon that has been rising during the past decade (Tayag, 2017). For popularity, we found that a song's loudness, danceability, instrumentalness, acousticness, and energy had associations with popularity. Loudness produces the biggest impact in popularity, and energy and instrumentalness were the only two variables that effected decreases in popularity. In addition, our chosen predictors could only explain 31% of the variability in popularity. For the predicting explicitness, all predictors (tempo, speechiness, danceability, energy) were significant. Two important results were decreases in tempo, indicating a song being 0.996 times less likely to be explicit, and increases in danceability, making a song 1.022 times more likely to be explicit. The notability of the extremes of these results is how minimal their impact is in affecting a song's likelihood to be explicitn, despite being statistically significant. In addition, our out-of-sample accuracy conducted on a test set from 2018-2019 revealed, at a 0.5 threshold cutoff, an overall accuracy of 81% with a 14% success rate of identifying explicit songs. In contrast, when adjusting the cutoff to the best threshold of 0.087, the overall accuracy dropped to 72%, but the ability to predict explicit songs rose to 73.2%. For non-explicit songs, the identification accuracy dropped from 97.9% to a 71.8%. These numbers driving this behavior were expected due to having such an extremely high number of non-explicit songs in our dataset. Lastly, the AUC value was a good 79% when grouping positives and negatives, and this score is relatively high. With these results, we feel inclined to try different predictors and time periods for future studies to improve the first model's R-squared value and the second model's accuracy, as well as exploring new response variables within Spotify's dataset and beyond it (other streaming giants such as Apple Music).
...
modeldf <- df1
colnames(modeldf) <- c("popularity","acousticness_og","danceability_og","energy_og","instrumentalness_og","tempo_og","loudness_og","speechiness_og")


#divide first
modeldf$danceability0.1 <- (modeldf$danceability_og)*10
modeldf$speechiness0.1 <- (modeldf$speechiness_og)*10
modeldf$energy0.001 <- (modeldf$energy_og)*1000
modeldf$instrumentalnes0.001 <- (modeldf$instrumentalness_og)*1000


#centered
modeldf$acousticness <- scale(modeldf$acousticness_og, scale = FALSE)
modeldf$danceability <- scale(modeldf$danceability_og, scale = FALSE)
modeldf$energy <- scale(modeldf$energy_og, scale = FALSE)
modeldf$instrumentalness <- scale(modeldf$instrumentalness_og, scale = FALSE)
modeldf$tempo <- scale(modeldf$tempo_og, scale = FALSE)
modeldf$loudness <- scale(modeldf$loudness_og, scale = FALSE)
modeldf$speechiness <- scale(modeldf$speechiness0.1, scale = FALSE)

modeldf

#backward
scaled1 <- lm(popularity ~ acousticness+danceability+energy+instrumentalness+tempo+loudness+speechiness, data = modeldf)
...
influential <- as.numeric(names(cooksd)[(cooksd > (4/nrow(modeldf)))])
modeldf_noinfluential <- modeldf[-influential, ]

model_noinfluential <- lm(popularity ~ acousticness+danceability+energy+instrumentalness+tempo+loudness+speechiness, data = modeldf_noinfluential)
...
**Testing the Model's Out-of-Sample Accuracy**
...
```{r warning=FALSE, echo=FALSE, message = FALSE,out.width = "50%", fig.show="hold", warning = FALSE, fig.align = "center"}
# test_fitted_0 <- predict(r2, newdata = test, type = "response") # Change Here
# test_fitted <- ifelse(test_fitted_0 >= 0.5, "Explicit", "Non-Explicit") #Change here
# Conf_mat3 <- confusionMatrix(as.factor(test_fitted), test$explicit_fac)
# ConfMat3 <- as.data.frame.matrix(Conf_mat3$table)
...
For the first research question, even after removing the influential points, we observe the Adjusted-Rsquared value increases to only 0.3059. This could be a potential limitation as the predictors can only explain variability in the outcome, the popularity score, up to 30.59%. This indicates a possibility of other significant predictors, which have not been taken into account for the analysis. For example, besides the attributes of a track, popularity could also be quite dependent largely on who is the artist. Secondly, the different genres have different combinations of musical attributes that make them popular within their respective frameworks. Therefore, this analysis doesn't integrate genre classification into its context. Thirdly, the interest of our research deals with the 2010s decade and cannot be extrapolated to the popularity of a track from other timeframes. As, music is a very dynamic research area, the attributes that appeal to people in recent times may not have been significant in an earlier time period. Lastly, we initially theorized that energy and tempo seem to have the similar kinds of statistical impact, yet, according to the model analysis, while energy is an important and significant attribute, tempo is not. Also, even though it seems that popular songs should have higher energy levels to drive higher engagement with the audience, our model analysis suggests otherwise. These discrepancies might indicate a possibility of an interaction term, which we haven't considered in the analysis.

For the second research question, despite providing insights toward predicting explicitness, the analysis indeed has some limitations. First, our model is weak at identifying non-explicit songs, probably because there was a smaller distribution of explicit songs in out dataset, which is reflective of real world patterns in regards to music consumption. The majority of songs are simply non-explicit; however, such distribution might hinder the training process when desiring to correctly categorize explicit and non-explicit songs. Secondly, more or better predictors may have been needed for this particular research question, such as the language of the song, its genre, popularity, duration, amongst others. Thirdly, Spotify likely employs the use of speech recognition (natural language processing) models to flag the explicitness of songs, but we used statistical methods. Thus, the research where we used the four predictors to statistically predict the nature of a such a "latent" variable may have already been fraught with much difficulty since the beginning. To conclude, these results do prove that any variant of our model, even after adjusting the thresholds, is insufficient for predicting the explicitness of songs (there are 19% of errors in predicting for the former and 28% for the latter), but, at least, it is overall successful when classifying the explicitness and non-explicitness of songs. For this second question,attempting to predict explicitness while overlooking the fundamental human element. Songwriters are the people who are making songs explicit and non-explicit. 
...
**Appendix 3: Binned Residual Plot**
