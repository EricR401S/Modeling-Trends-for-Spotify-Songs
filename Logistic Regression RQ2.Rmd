...
test_fitted <- ifelse(test_fitted_0 >= 0.5, "Non-Explicit", "Explicit") #Change here
...
Lastly, we look at the final and most important piece, the accuracy of our model on the test set. Shown below are the ROC curve and the confusion matrix, as well as the overall accuracy values. One of our challenges was addressing the small number of explicit songs in our data, where it might hurt model accuracy. That concern was proven true when our model failed at correctly classifying 25,822 non-explicit songs and 762 explicit songs. It had an overall, low accuracy of 18.4%, a sensitivity of 1.7% and a specificity of 87.8%. The sensitivity value signifies that our model is inefficient at identifying non-explicit songs, while our specificity value identifies the opposite, where our explicit songs are classified with greater accuracy.


NOTE : Ask Andrea about this oddity. Shouldn't our model be terrible at identifying explicit songs, not non-explicit? I checked the distribution (hard numbers) of our test set, and they looked reasonable. Sensitivity is true positives, and specificity is true negatives. 
